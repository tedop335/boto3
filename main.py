...
parser.add_argument("-usf",
                    "--upload_small_file",
                    type=str,
                    help="Path to the small file to upload.",
                    default=None)

parser.add_argument("-ulf",
                    "--upload_large_file",
                    type=str,
                    help="Path to the large file to upload.",
                    default=None)

parser.add_argument("-mvt",
                    "--mimetype_validation",
                    type=str,
                    help="Expected mimetype for file validation.",
                    default=None)
...
def main():
  ...
  if args.bucket_name:
    ...
    if args.upload_small_file:
      if args.mimetype_validation:
        mimetype, _ = mimetypes.guess_type(args.upload_small_file)
        if mimetype != args.mimetype_validation:
          parser.error(f"Mimetype validation failed. Expected {args.mimetype_validation}, got {mimetype}")
    ...
    if args.upload_large_file:
      if args.mimetype_validation:
        mimetype, _ = mimetypes.guess_type(args.upload_large_file)
        if mimetype != args.mimetype_validation:
          parser.error(f"Mimetype validation failed. Expected {args.mimetype_validation}, got {mimetype}")
